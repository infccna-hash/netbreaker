-- 052_lab16_harden_dedupe_recovery.up.sql
--
-- Corrective hotfix: migration 050's harden UPDATE used string
-- concatenation (content = $prepend$...$prepend$ || content), which
-- is not idempotent. The recovery block ended up prepended twice in
-- the live DB. This sets harden content directly (plain assignment,
-- not concatenation) to the single-prepend state that was intended,
-- regardless of how many times 050 actually ran.

BEGIN;

UPDATE lab_phases SET
  content = $md$<div class="mission">
<span class="tag">🔧 RECOVERY</span>
<h3>Restore the link first</h3>
<p>Before you lock anything down, undo the damage — bring PC1's port back up:</p>

configure terminal interface et0/0 no shutdown end show interfaces et0/0 | include line protocol
...
<div class="achievement"><span class="medal">🛡</span><span class="txt"><span class="lbl">Achievement Unlocked</span><span class="name">Physical Guardian — the network is now as secure as the cable plant</span></span></div>$md$
