-- ═══════════════════════════════════════════════════════════════════
-- Lab 02 (id=2) — Add port labels to topology SVG
-- ═══════════════════════════════════════════════════════════════════
-- This migration:
--   1. Verifies the current SVGs match expected old values
--   2. Replaces them with new SVGs that include port labels on all links
--   3. Also fixes a latent SW3 label bug (still said "Fa0/2" instead of "Et0/2")
--
-- Port mapping (console-truth, verified 2026-08-03):
--   SW1 Et0/0 ↔ SW2 Et0/1   (inter-switch — moved from Et0/1 to resolve conflict)
--   SW1 Et0/2 ↔ SW3 Et0/1   (inter-switch)
--   SW2 Et0/2 ↔ SW3 Et0/3   (inter-switch, blocked by STP)
--   SW1 Et0/1 ↔ PC1          (host)
--   SW3 Et0/2 ↔ KALI         (host/attacker)

-- Phase 1: Verify old SVGs are what we expect
DO $$
DECLARE
  current_small text;
  current_large text;
  expected_small text := $old_small$<svg viewBox="0 0 320 220" xmlns="http://www.w3.org/2000/svg" font-family="ui-monospace, monospace">
  ...
  expected_large text := $old_large$<svg viewBox="0 0 760 460" xmlns="http://www.w3.org/2000/svg" font-family="ui-monospace, monospace">
...
BEGIN
  ...
  IF current_small IS NULL THEN
    RAISE EXCEPTION 'lab 2 svg_small is NULL';
  END IF;
  IF current_large IS NULL THEN
    RAISE EXCEPTION 'lab 2 svg_large is NULL';
  END IF;
  IF current_small != expected_small THEN
    RAISE EXCEPTION 'lab 2 svg_small changed since migration 009 — aborting. Run a dry run first.';
  END IF;
  IF current_large != expected_large THEN
    RAISE EXCEPTION 'lab 2 svg_large changed since migration 009 — aborting. Run a dry run first.';
  END IF;
END $$;

-- Phase 2: Update with new SVGs that include port labels
